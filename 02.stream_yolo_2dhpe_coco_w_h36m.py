...
import numpy as np
from utils import h36m_coco_format, drawkeypoints, drawkeypoints2
...
while True:
    ...
    try:
        ...
        for result in results:
            ...
            if scores.any():
                frame = drawkeypoints(frame, jointsdata, color=(255, 0, 0), thickness=8)
                h36m_kpts, h36m_scores, valid_frames = h36m_coco_format(keypoints, scores)
                kpc = np.concatenate((h36m_kpts, h36m_scores), axis=-1)
                frame = drawkeypoints2(frame, kpc)  # 스켈레톤 그리기
